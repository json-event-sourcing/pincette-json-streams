...
class Build implements Runnable {
  ...
  private static class CollectionOrLocal {
    @Option(
        names = {"-c", "--collection"},
        description = "The MongoDB collection the apps are uploaded to.")
    private String collection;

    @Option(
        names = {"-l", "--local"},
        description =
            "Sends the build result to stdout, even when the configuration contains an "
                + "app collection.")
